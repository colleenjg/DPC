...
class GaborConfusionMeter(loss_utils.ConfusionMeter):
    ...
    def plot_mat(self, save_path=None, nest_frames=False, title=None):
        ...
        try:
            ...
            fig = super().plot_mat(adj_aspect=True, **cbar_kwargs)
...
def get_gabor_sup_target_to_store(dataset, sup_target):
    ...
    into (image type, orientation (in degrees)).
...
def get_gabor_sup_label(sup_target, warn=False):
    ...
    - sup_target: torch Tensor
        ...
        with dims: 
            ...
            dimension is [label, unexp_label]
...
def plot_save_gabor_conf_mat(gabor_conf_mat, mode="train", epoch_n=0, 
    ...
    object, and saves its storage dictionary to a json by adding it to an 
    existing file, if it exists, or creating a new one.
    ...
    - nest_frames : bool (default=False)
        ...
        instead of preserving the class label order.
    - output_dir : str or path
        Directory in which to save the plotted confusion matrix. Also, the 
        directory from which to load the json file, if it exists, and where the 
        updated or new json file is saved.
    """
...
#############################################
def load_replot_gabor_conf_mat(gabor_conf_mat_dict_path, unexp_epoch=None, 
                               U_prob=0.1, nest_frames=False, output_dir=None, 
                               parallel=False):
    """
    load_replot_gabor_conf_mat(gabor_conf_mat_dict_path)

    Loads and replots Gabor Gabor confusion matrices from a json, in which they 
    are stored by epoch.
    ...
    - output_dir : str or path (default=".")
        Directory in which to save the plotted confusion matrices.
...
def init_gabor_records(dataset, init_conf_mat=True):
    ...
    -------
    - loss_dict : dict
        Dictionary for collecting loss data, with one key per gabor image type 
        (e.g., "A", "B", etc.) and one key per gabor orientation 
        (e.g., 0, 180, "N/A").
    - acc_dict : dict
...
def update_records(dataset, loss_dict, acc_dict, output, sup_target, 
    ...
    - dataset : gabor_stimuli.GaborSequenceGenerator
        Gabor dataset
    - loss_dict : dict
        Dictionary to update (in place) with loss data, with one key per gabor 
        image type (e.g., "A", "B", etc.) and one key per gabor orientation 
        (e.g., 0, 180, "N/A").
    - acc_dict : dict
    ...
    - output : nd array
        Model output. 
        If supervised, the dimensions are B (batch size) x number of classes. 
        Otherwise, the dimensions are B (batch_size) x PS (predicted step)
    - sup_target : nd array
        Supervised target returned by the Gabors dataset for each sequence, 
        with dims: 
            B (batch size) x N (num seq) x SL (seq len) x 2, where the last 
            dimension is [label, unexp_label]
    - batch_loss : nd array
        Loss for each image. If supervised, the dimensions are B (batch size). 
        Otherwise, the batch loss is averaged across the spatial dimension, 
        and the dimensions are B (batch size) x PS (prediction step).
