...
class GaborConfusionMeter(loss_utils.ConfusionMeter):
    ...
    - self.get_norm_ori_mat()
        Returns a copy of self.mat, normalized for relative mean orientation 
        frequency. Specifically, it is normalized for the ("G", "N/A") 
    ...
    def get_norm_ori_mat(self):
        ...
        Normalization is calculated using the expected relative frequency of 
        the "N/A" mean orientation (which applies to all "G" images) compared 
        to other orientations for all other frames, in both prediction and 
        ground-truth data.

        Returns
        -------
        - norm_ori_mat : 2D array
            Normalized confusion matrix, with dimensions 
            self.num_classes x self.num_classes. 
        """

        class_name = ("G", "N/A")
        norm_ori_mat = copy.deepcopy(self.mat)
        if class_name in self.class_names:
            idx = self.class_names.index(class_name)
            rel_freq = len(self.mean_oris) - 1
            norm_ori_mat[idx] = (norm_ori_mat[idx] / rel_freq).astype(int)
            norm_ori_mat[:, idx] = (norm_ori_mat[:, idx] / rel_freq).astype(int)
    ...
    def plot_mat(self, save_path=None, nest_frames=False, title=None, 
        ...
        - title : str (default=None)
            Plot title.
        - norm_G : bool (default=True)
            If True, counts for the (G, "N/A") class are normalized to be 
            comparable to the frequency of the other classes for plotting 
            purposes.
        ...
        try:
            ...
            if norm_ori:
                self.mat = self.get_norm_ori_mat()
        ...
        if norm_ori:
            fig.axes[-1].set_ylabel("Counts (norm. by ori. freq.)")
